...
/// then called as `__chromiumctl_deepQuerySelector(document, selector)`.
...
pub fn deep_query_selector_js() -> &'static str {
    r#"function __chromiumctl_deepQuerySelector(root, selector) {
        var direct = root.querySelector(selector);
        if (direct) return direct;
        var hosts = root.querySelectorAll('*');
        for (var i = 0; i < hosts.length; i++) {
            if (hosts[i].shadowRoot) {
                var found = __chromiumctl_deepQuerySelector(hosts[i].shadowRoot, selector);
...
mod tests {
    ...
    fn test_deep_query_selector_js_declares_the_expected_function_name() {
        ...
        assert!(js.contains("function __chromiumctl_deepQuerySelector(root, selector)"));
    }
